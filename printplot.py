...
if(i_want_to_compile_and_run):
    os.system("g++ -O0 cache_measurment.cpp -o cache_measurment")#compile program without optimization
    os.system("./cache_measurment > cache_measurment.out")

x=np.loadtxt("cache_measurment.out",delimiter='\t',skiprows=1)
y=x[:,0]
x=x[:,1]

#some noise filtering
n = 1  # the larger n is, the smoother curve will be. If n is 1, you will get non-filtered plot
b = [1.0 / n] * n
a = 1
x = lfilter(b,a,x)
